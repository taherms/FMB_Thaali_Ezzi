...
Features
--------
- Firebase Authentication for sign in and sign up (first account becomes admin)
- Admins assign delivery turns to eligible members on a given date, reassign, mark
  complete, or delete turns from the Assign Turns tab
...
- Each member manages their own name, phone, and drop-off address from My Address
...
5. In Firestore, these collections are used (created automatically as the app writes
   ...
   - users            (profile, role, kidmat eligibility, address)
...
6. Recommended Firestore security rules (set these in the Firebase console —
   ...
   - A user may create/update only their own `users/{uid}` document, except the
     `role` and `eligibleForKidmat` fields, which only an admin may change.
...
2. Every member sets their own name, phone, and drop-off address in My Address.
3. Admins assign delivery turns to eligible members from Assign Turns, and can
   promote/demote admins or toggle kidmat eligibility from Admin > Members.
